        ...
        {rows.map(r => (
          <div key={r.ah.id}>
            ...
            <div className="w-full bg-slate-100 rounded-full h-1.5">
              <div
                className={`h-1.5 rounded-full transition-all duration-700 ${pctColor(r.thisPct)}`}
                style={{ width: `${r.thisPct}%` }}
              />
            </div>
          </div>
        ))}
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center animate-fade-in">
      ...
      <button
        onClick={onAdd}
        className="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white text-sm font-medium px-5 py-2.5 rounded-xl transition-colors"
      >
